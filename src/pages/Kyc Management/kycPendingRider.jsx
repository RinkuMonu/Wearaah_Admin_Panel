...
export default function PendingKycRiderList() {
  ...
  const handleToggleStatus = async (rider, isChecked) => {
    ...
    const result = await Swal.fire({
      ...
      text: `Are you sure you want to ${isChecked ? "activate" : "deactivate"} this rider?`,
      icon: "warning",
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            type="checkbox"
                            checked={rider.user?.isActive}
                            disabled={statusLoading === rider?.user?._id}
                            onChange={(e) =>
                              handleToggleStatus(rider, e.target.checked)
                            }
                            className="sr-only peer"
                          />

                          <div className="w-11 h-6 bg-gray-300 peer-checked:bg-green-600 rounded-full transition-all"></div>
