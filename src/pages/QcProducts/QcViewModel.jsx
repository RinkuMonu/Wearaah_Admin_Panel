...
export default function VariantQCModal({ productId, productName, onClose, onSuccess }) {
    ...
    const handleApprove = async (variant) => {
      ...
      `,
        ...
        if (result.isConfirmed) {
            setSubmitting(true);
            try {
                const res = await api.put(`/variant/admin/variant/${variant._id}/status`, {
                    status: "approved",
                    qcNote: ""
                });
    ...
    const getStatusBadge = (status, isActive) => {
        ...
        const statusConfig = {
            ...
            pending: { color: "bg-yellow-100 text-yellow-800", icon: Clock, label: "Pending QC" },
    ...
    const nextImage = () => {
        if (selectedVariant?.variantImages?.length > 0) {
            setCurrentImageIndex((prev) => (prev + 1) % selectedVariant.variantImages.length);
        }
    };

    const prevImage = () => {
        if (selectedVariant?.variantImages?.length > 0) {
            setCurrentImageIndex((prev) => (prev - 1 + selectedVariant.variantImages.length) % selectedVariant.variantImages.length);
        }
    };

    const filteredVariants = variants.filter(variant => {
        const matchesSearch = variant.variantTitle.toLowerCase().includes(searchTerm.toLowerCase()) ||
            variant.sku.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesStatus = filterStatus === "all" || variant.status === filterStatus;
        return matchesSearch && matchesStatus;
    });

    const pendingCount = variants.filter(v => v.status === "pending" && v.isActive).length;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden">
                ...
                <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
                    <div>
                        <h2 className="text-xl font-bold text-gray-800">Variant Quality Check</h2>
                        <p className="text-sm text-gray-500">
                            {productName} • {variants.length} variant(s) • {pendingCount} pending review
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    <div className="flex gap-2">
                        ...
                        {selectedVariant && (
                            <button
                                ...
                                className={`px-4 py-2 text-sm font-medium transition-colors ${activeTab === "detail"
                                    ? "text-blue-600 border-b-2 border-blue-600"
                                    : "text-gray-500 hover:text-gray-700"
                                    }`}
                            >
                            ...
                            {filteredVariants.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {filteredVariants.map((variant) => (
                                        <div
                                            key={variant._id}
                                            className={`border rounded-lg p-4 cursor-pointer transition-all hover:shadow-md ${selectedVariant?._id === variant._id
                            ...
                            <div className="p-6">
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-4">
                                        <div className="relative bg-gray-100 rounded-lg overflow-hidden aspect-square">
                                            {selectedVariant.variantImages?.length > 0 ? (
                                                <>
                                                    <img
                                                        src={baseUrl + selectedVariant.variantImages[currentImageIndex]}
                                            ...
                                            <div className="flex gap-2 overflow-x-auto pb-2">
                                                {selectedVariant.variantImages.map((img, idx) => (
                                                    <button
                                                        ...
                                                        className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 ${idx === currentImageIndex ? 'border-blue-500' : 'border-gray-200'
                                                            }`}
                                                    >
                                                        <img
                                                            src={baseUrl + img}
                                    ...
                                    <div className="space-y-6">
                                        {/* Variant Info */}
                                        <div>
                                            <h3 className="text-xl font-bold text-gray-800">{selectedVariant.variantTitle}</h3>
                                            <p className="text-sm text-gray-500 mt-1">SKU: {selectedVariant.sku}</p>
                                            <div className="mt-2">{getStatusBadge(selectedVariant.status, selectedVariant.isActive)}</div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <div className="bg-gray-50 rounded-lg p-3">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    />
                                                    <p className="font-semibold text-gray-800 capitalize">{selectedVariant.color}</p>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-gray-50 rounded-lg p-4">
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex justify-between text-sm">
                                                    ...
                                                    <span className="font-medium">{formatCurrency(selectedVariant.pricing?.costPrice)}</span>
                                                </div>
                                                <div className="flex justify-between text-sm">
                                                    <span className="text-gray-600">MRP:</span>
                                                    <span className="font-medium line-through text-gray-500">{formatCurrency(selectedVariant.pricing?.mrp)}</span>
                                                </div>
                                                <div className="flex justify-between text-sm">
                                                    <span className="text-gray-600">Selling Price:</span>
                                                    <span className="font-bold text-green-600">{formatCurrency(selectedVariant.pricing?.sellingPrice)}</span>
                                                </div>
                                                <div className="flex justify-between text-sm">
                                                    <span className="text-gray-600">Stock:</span>
                                                    <span className={`font-medium ${selectedVariant.stock === 0 ? 'text-red-600' :
                                                ...
                                                <div className="flex justify-between text-sm">
                                                    ...
                                                    <span className="font-medium">{selectedVariant.returnable ? "Yes" : "No"}</span>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                            ...
                                            <ul className="space-y-2 text-sm text-blue-700">
                                                <li className="flex items-start gap-2">
                                                    ...
                                                    <span>Images are clear and show the actual product</span>
                                                </li>
                                                <li className="flex items-start gap-2">
                                                    <CheckCircle className="w-3 h-3 mt-0.5 flex-shrink-0" />
                ...
                {/* Footer Actions */}
                {activeTab === "detail" && selectedVariant && selectedVariant.status === "pending" && selectedVariant.isActive && (
